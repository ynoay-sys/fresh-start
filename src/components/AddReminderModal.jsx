...
export default function AddReminderModal({ onClose, onSaved }) {
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" dir="rtl">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <h2 className="font-bold text-gray-900">הוספת תזכורת אישית</h2>
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">תאריך ושעה *</label>
            <input type="datetime-local" value={scheduledFor} onChange={e => setScheduledFor(e.target.value)}
              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-blue-400" />
          </div>
        </div>
        <div className="px-6 py-4 border-t border-gray-100 flex gap-3">
          <button onClick={handleSave} disabled={saving || !title.trim() || !scheduledFor}
