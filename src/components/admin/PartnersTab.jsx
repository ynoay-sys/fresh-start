...
import { Search, Plus, Trash2, ShieldCheck, Star, PauseCircle, Check, X } from "lucide-react";
...
export default function PartnersTab() {
  ...
  async function load() {
    setLoading(true);
    const [pArr, prArr, appArr] = await Promise.all([
      base44.entities.ProfessionalPartner.list(),
      base44.entities.UserProfile.filter({ role: "partner" }),
      base44.entities.PartnerApplication.filter({ status: "pending" }),
    ]);
    setPartners(pArr);
    setProfiles(prArr);
  ...
  async function handleApproveApplication(app) {
    await base44.entities.PartnerApplication.update(app.id, { status: "approved", reviewed_at: new Date().toISOString().split("T")[0] });
    // Create ProfessionalPartner record
    await base44.entities.ProfessionalPartner.create({
      name: app.applicant_name, email: app.applicant_email,
      profession: app.profession, city: app.city, phone: app.phone,
      description: app.bio, website: app.website,
      is_active: true, is_verified: false, plan: "free",
      profile_views: 0, contact_requests_count: 0,
      logoInitials: app.applicant_name?.slice(0, 2) || "?",
    });
    load();
  }

  async function handleRejectApplication(app) {
    await base44.entities.PartnerApplication.update(app.id, { status: "rejected", reviewed_at: new Date().toISOString().split("T")[0] });
    load();
  }
    ...
    <div>
      ...
      <div className="flex gap-1 mb-5 bg-gray-100 p-1 rounded-xl w-fit">
        <button onClick={() => setActiveTab("active")}
          className={`px-5 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === "active" ? "bg-white shadow text-gray-900" : "text-gray-500 hover:text-gray-700"}`}>
          שותפים פעילים ({partners.length})
        </button>
        <button onClick={() => setActiveTab("pending")}
          className={`px-5 py-2 rounded-lg text-sm font-medium transition-colors relative ${activeTab === "pending" ? "bg-white shadow text-gray-900" : "text-gray-500 hover:text-gray-700"}`}>
          בקשות ממתינות
          {pendingCount > 0 && (
            <span className="mr-1.5 inline-flex items-center justify-center w-5 h-5 rounded-full text-white text-xs font-bold" style={{ backgroundColor: "#EF4444" }}>
              {pendingCount}
            </span>
          )}
        </button>
      ...
      {activeTab === "pending" && (
        <div className="bg-white rounded-xl border border-gray-200 overflow-x-auto">
          {loading ? (
            ...
            <div className="text-center py-12 text-gray-400">אין בקשות ממתינות</div>
          ) : (
            <table className="w-full text-sm" dir="rtl">
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>
                  {["שם / אימייל", "מקצוע", "עיר", "תאריך הגשה", "פעולות"].map(h => (
                    <th key={h} className="px-4 py-3 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider">{h}</th>
                  ))}
                </tr>
              </thead>
                ...
                {applications.map(app => (
                  <tr key={app.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3">
                      ...
                      <p className="text-xs text-gray-400" dir="ltr">{app.applicant_email}</p>
                    </td>
                    <td className="px-4 py-3 text-gray-600">{app.profession || "—"}</td>
                    <td className="px-4 py-3 text-gray-600">{app.city || "—"}</td>
                    <td className="px-4 py-3 text-gray-500 text-xs">{app.submitted_at || "—"}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => handleRejectApplication(app)}
